---
title: Using key prop when mapping through list of items in React
...
Even if you worked with React for a long time I am certain you see this warning form time to time. No shame in that we all make mistakes 😅. Let's learn why it's so important to have `key`s for our lists.
...
React compares the elements by shallow comparison using `Object.is` as it is safer way for React than using `===`. The differences between `===` and `Object.is` is that `Object.is` treats `NaN`s as the same values and `0` and `-0`as different. See [MDN documentation on `Object.is` if you want to know more](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/is) <NewTabIcon/>. Then there is the JSX syntax that tells React what to render and it is basically a `createElement` function call to add elements to the page if their type or `key` changed.

Where `keys` play an instrumental role is the described above reconciliation mode. Reconciliation happens when React verifies if the type of the component changed and the old one should be removed (unmounted) from the DOM or if it needs to update the component (when props change). See: [rendering lists](https://react.dev/learn/rendering-lists) <NewTabIcon/>
...
This use of `createElement` is why we cannot have conditional logic inside `JSX` but that is a topic for another article 😉.
...
## Complex example with components, memo and keys
