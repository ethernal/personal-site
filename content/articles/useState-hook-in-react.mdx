...
- Initialize state with initial value or updater function.
- Change (mutate) current state and use variables to access new state before it updates.
- Change current state based on pending state.
- Store objects in state and update their data when one part is replaced.
