...
authorId: sebee.website@gmail.com
...
Example of passing single read only value to context from React documentation:
...
The part where we retrieve the `theme` variable from the cookies and set it while the page is still rendering on the server is the secret sauce to preventing content flashing as it will always be in sync with that the user sets on the client (soon) and by utilizing Tailwind we are sure that all the CSS we need for the page is already present.
