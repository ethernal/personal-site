import format from 'date-fns/format';
import Link from 'next/link';
import React from 'react';

import Logo from '@/components/Logo';
import { SITE_PUBLISHED_YEAR_STRING, SOCIAL_X_HANDLE } from '@/constants/constants';

// import styles from './Footer.module.css';

type FooterProps = {
	className?: string;
};

function Footer({ className }: FooterProps) {
	const currentYear = format(new Date(), 'yyyy');

	return (
    <div className={className}>
      <div
        className={
          "md:jstify-between max-w-content-width relative flex w-full flex-row justify-between gap-16 pb-12 max-md:gap-8 max-xs:flex-col md:flex-row md:items-baseline"
        }
      >
        <div className="flex flex-col">
          <Logo className={"text-xl"} />
          <ul className={"list-none p-0 first:mt-6 last:mb-0"}>
            <li className="mb-2">
              <p className={"mb-0 mt-7 text-xs font-light text-decoration"}>
                Inspired by {` `}
                <a
                  href="https://www.joshwcomeau.com/"
                  className={
                    "font-semibold text-slate-900 no-underline hover:underline hover:decoration-2 hover:underline-offset-[0.125em]"
                  }
                >
                  Josh W. Comeau
                </a>
                .
              </p>
            </li>
            <li className="mb-2">
              <p className={"mb-0 mt-4 text-sm text-decoration"}>
                Created by{" "}
                <Link
                  href="/"
                  className={
                    "font-semibold text-slate-900 no-underline hover:underline hover:decoration-2 hover:underline-offset-[0.125em]"
                  }
                >
                  Sebastian Pieczyński
                </Link>{" "}
                &copy;{" "}
                {SITE_PUBLISHED_YEAR_STRING === currentYear ? "" : "2023-"}
                {currentYear}.{" "}
              </p>
            </li>
          </ul>
        </div>

        <nav>
          <h2 className={"mt-0 text-xl"}>Links</h2>
          <ul className={"mt-6 list-none p-0 last:mb-0"}>
            <li className="mb-2">
              <Link className="text-base no-underline" href="/terms">
                Terms of Use
              </Link>
            </li>
            <li className="mb-2">
              <Link className="text-base no-underline" href="/privacy">
                Privacy Policy
              </Link>
            </li>
            <li className="mb-2">
              <a
                className="text-base no-underline"
                href={`https://twitter.com/${SOCIAL_X_HANDLE}`}
              >
                Twitter / X
              </a>
            </li>
          </ul>
        </nav>
      </div>
    </div>
  );
}

export default Footer;
